...
## Minimum volume (in dB) required to trigger transcription. 
## Adjust this: -10 is very loud, -60 is very quiet. Try around -40.
@export var volume_threshold_db := -22.0
...
func transcribe_thread():
	...
	while recording:
		...
		if resampled.size() <= 0:
			...
			continue
		# Cek volume dari buffer saat ini (resampled)
		var current_db = _get_rms_db(resampled)
		
		# Jika volume di bawah threshold, kita anggap sepi/kecil
		if current_db < volume_threshold_db:
			# Clear accumulated frames to prevent buffer from growing indefinitely
			# Keep a small tail (0.5s) to avoid cutting off speech that starts quietly
			var mix_rate : int = ProjectSettings.get_setting("audio/driver/mix_rate")
			var tail_samples = int(0.5 * mix_rate)
			if _accumulated_frames.size() > tail_samples:
				_accumulated_frames = _accumulated_frames.slice(_accumulated_frames.size() - tail_samples)
			
			# Delay sedikit dan skip proses transkripsi berat
			OS.delay_msec(100) 
			continue	
...
func _get_rms_db(buffer: PackedFloat32Array) -> float:
	var sample_count = buffer.size()
	if sample_count == 0:
		return -80.0 # Silence
	
	var sum_squares = 0.0
	for i in range(sample_count):
		# Karena data mono (float), cukup kuadratkan nilainya langsung
		sum_squares += buffer[i] * buffer[i]
	
	var rms = sqrt(sum_squares / sample_count)
	
	# Konversi amplitudo linear ke Desibel
	print(linear_to_db(rms))
	if rms > 0:
		return linear_to_db(rms)
	return -80.0 # Floor noise (hening total)
